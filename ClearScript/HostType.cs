...
namespace Microsoft.ClearScript
{
    internal class HostType : HostTarget, IScriptableObject
    {
        ...
        public override bool TryInvokeAuxMember(string name, BindingFlags invokeFlags, object[] args, out object result)
        {
            ...
            if (type != null)
            {
                ...
                if (nestedTypes.Count > 0)
                {
                    ...
                    {
                        return tempResult.TryInvoke(invokeFlags, args, out result);
                    }
        ...
        public override bool TryInvoke(BindingFlags invokeFlags, object[] args, out object result)
        {
            ...
            var template = templates.FirstOrDefault(testTemplate => testTemplate.GetGenericParameterCount() == typeArgs.Length);
            if (template == null)
            {
